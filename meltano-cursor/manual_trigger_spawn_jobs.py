        ...
        "containers": [
            {
                ...
                "args": [
                    "-c",
                    "env > /etc/environment && echo $SERVICE_ACCOUNT_KEY_CONTENT >  /etc/service_account_key_content && chmod +x /tmp/git/monorepo/meltano-mysql-snowflake-job/run_prod_hp_cursor.sh && /tmp/git/monorepo/meltano-mysql-snowflake-job/run_prod_hp_cursor.sh",
                ],
                "volumeMounts": [
                    {
                        "name": "custom-secret",
...
if __name__ == "__main__":
    ...
    job_id = spanwn_job(f"{schema}-{name}", schema, sync_reg, name, "UPDATED_AT")
    if job_id:
