...
def clean_up_csv():
    df = pd.read_csv('Viper DB Tables For Syncing - Aug 2024 - Prod.csv')
    df = df[df['Method'] == 'Log-Based Incremental']
    logging.info(f"Loaded {len(df)} tables for processing")
    return df


def spanwn_job(spanwed_name, schema, sync_reg, name):
    job_name = f"prod_{STAMP}"
        ...
        "containers": [
            {
                ...
                "env": [
                    ...
                    {"name": "PIPELINES_USER", "value": "stella"},
                    {
                        ...
                        "value": "meltano-mysql-snowflake-job/run_prod.yaml",
                    },
                    {
                    ...
                    },
                    {
                        ...
                        # "value": random.choice(["10.64.9.59", "10.64.4.144", "10.64.7.210"])
                        "valueFrom": {
                            "secretKeyRef": {
                                "name": "source-mysql-secret-qa",
                    ...
                    {
                      "name": "HYPERPLANE_JOB_PARAMETER_EXTRA_FLAG",
                      "value": " --full-refresh"  
                    },
                    {
                ...
                "args": [
                    "-c",
                    "env > /etc/environment && echo $SERVICE_ACCOUNT_KEY_CONTENT >  /etc/service_account_key_content && chmod +x /tmp/git/monorepo/meltano-mysql-snowflake-job/run_prod.sh && /tmp/git/monorepo/meltano-mysql-snowflake-job/run_prod.sh",
                ],
                "volumeMounts": [
                    {
                        "name": "custom-secret",
    ...
    "variables": {
        ...
        "activeTimeout": -1,
        "maxRetries": 0,
        "customMessage": f'[prod], {schema}, {name}', 
        "yamlPath": "meltano-mysql-snowflake-job/run_prod.yaml",
        "noGitInit": False,
        "hyperplaneSecrets": [
        # {
        ...
        "id": "7bc78e07-709e-4c0d-8490-167c23947208"
        },
        "branchName": "nat_test",
        "commitId": "",
        "parameters": {
    ...
    if response.status_code == 200:
        ...
        if job_id:
            ...
            logging.error(f"Failed to spawn job with regex with status code 200: {sync_reg}")
            logging.error(f"Response: {json.dumps(response.json(), indent=4)}")
            return None
    else:
        logging.error(f"Failed to spawn job {sync_reg} with status code {response.status_code}. Response: {response.text}")
        return None

if __name__ == "__main__":
    schema = "MELTANO_VIPER_PROD_HIGH_PRIORITY"
    name = "stingray-voucher"
    sync_reg = "stingray-voucher.*"
    
    job_id = spanwn_job(f"{schema}-{name}", schema, sync_reg, name)
    if job_id:
        print(f"Job spawned successfully: {job_id}")
    else:
        print("Failed to spawn job")